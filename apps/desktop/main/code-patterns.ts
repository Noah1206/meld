...
/**
 * Collect code files from the project's source directories (up to maxFiles)
 */
function collectSourceFiles(projectPath: string, maxFiles: number): string[] {
  ...
  for (const dir of srcDirs) {
    ...
    walkDir(fullDir, candidates, maxFiles * 3); // Collect enough then trim later
    if (candidates.length >= maxFiles) break;
  }

  // Trim to include a variety of files
...
/**
 * Analyze code patterns in a single file
 */
function analyzeFile(filePath: string, result: PatternResult): void {
  let content: string;
  try {
    const stat = fs.statSync(filePath);
    if (stat.size > 100 * 1024) return; // Skip files over 100KB
...
/**
 * Format analysis results as a concise markdown string (max ~500 chars)
 */
function formatPatterns(result: PatternResult): string {
  ...
  if (result.typescript.propsNaming.length > 0) {
    // Summarize Props naming pattern (e.g., ComponentProps, ComponentNameProps)
  ...
  // Frequently used libraries (top 5)
  const topImports = Array.from(result.commonImports.entries())
    .filter(([pkg]) => !["react", "next"].includes(pkg)) // Exclude the framework itself
    .sort((a, b) => b[1] - a[1])
    .slice(0, 5)
    .map(([pkg]) => pkg);
...
/**
 * Analyze code patterns for a project path and return a markdown summary.
 * Samples and analyzes 5-10 source files.
 */
